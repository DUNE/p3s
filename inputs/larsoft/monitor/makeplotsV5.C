// makeplotsV5.C
// to execute non-interactively:  root -b -l -q 'makeplotsV5.C("rawtpcmonitor.root");'
// root -b -l -q 'makeplotsV5.C("rawtpcmonitor.root");' > /dev/null 2>&1
...
void makeplotsV5(TString infile="rawtpcmonitor.root"){
  ...
  for(int i=0; i < directories_in_file.size(); i++){
    ...
    if(dirstr.Contains("pdsphitmonitor") && CREATEHTMLFILES){
      //std::cout << "INFO::Attempting to plot from " << dirstr.Data() << std::endl;
    ...
    if(dirstr.Contains("sspmonitor") && CREATEHTMLFILES){
...
TObjArray* SaveHistosFromDirectory(TDirectory *dir, TString runname, TString datename){
  ...
  TString sourcefile_str = dirname  + TString(".html"); 
  FILE *sourcefile;
  if(CREATEHTMLFILES){
    sourcefile = fopen(sourcefile_str.Data(),"w");
  ...
  while((key = (TKey*)nextkey())){
	...
	cout << HistoNameOrig << endl;
	HistoNameOrig.ReplaceAll("0U","U0");
	HistoNameOrig.ReplaceAll("0V","V0");
	HistoNameOrig.ReplaceAll("0Z","Z0");
	HistoNameOrig.ReplaceAll("1U","U1");
	HistoNameOrig.ReplaceAll("1V","V1");
	HistoNameOrig.ReplaceAll("1Z","Z1");
	HistoNameOrig.ReplaceAll("2U","U2");
	HistoNameOrig.ReplaceAll("2V","V2");
	HistoNameOrig.ReplaceAll("2Z","Z2");
	HistoNameOrig.ReplaceAll("3U","U3");
	HistoNameOrig.ReplaceAll("3V","V3");
	HistoNameOrig.ReplaceAll("3Z","Z3");
	HistoNameOrig.ReplaceAll("4U","U4");
	HistoNameOrig.ReplaceAll("4V","V4");
	HistoNameOrig.ReplaceAll("4Z","Z4");
	HistoNameOrig.ReplaceAll("5U","U5");
	HistoNameOrig.ReplaceAll("5V","V5");
	HistoNameOrig.ReplaceAll("5Z","Z5");
	cout << HistoNameOrig << endl;
	cout << "-------------------------" << endl;
      }
